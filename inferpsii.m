...
for t = 1:s
  kt = size(Lm{t},2);
  temp_alt = Xm{t}.*repmat(sum(Qns(:,t,:),3)',kt,1);
  temp = Xcov{t}*sum(sum(Qns(:,t,:),3),1)  +  Xm{t}*temp_alt';
  if pcaflag == 0
    psi = psi + (repmat(sum(Qns(:,t,:),3)',p,1).*Y) * (Y-2*Lm{t}*Xm{t})' ...
	+ Lm{t}*temp*Lm{t}';
    for q = 1:p
      psi(q,q) = psi(q,q) + trace( Lcov{t}(:,:,q)*temp );
    end
  else
	zeta = zeta + ( + sum((Y.*(Y-2*Lm{t}*Xm{t}))*sum(Qns(:,t,:),3),1) ...
